...
def main(task: str):
  linkdti_data_predict = pd.read_csv(f'/Users/lewconb/2026 Project/LinkDTI_BBB/linkdti_bbb_data/drug_smiles_collation.csv', sep="\t")
  ...
  process_data(linkdti_data_predict, tokenizer, model, device, "linkdti_data_predict", task)
...
def process_data(df, tokenizer, model, device, set_name, task, batch_size=64):
  ...
  np.save(f'/Users/lewconb/2026 Project/LinkDTI_BBB/linkdti_bbb_data/features/{set_name}-text-{task}.npy', out)
