...
def load_data():
    print("Loading data...")
    network_path = 'LinkDTI_heterogenous_KG_DA_project/data/'
...
# Step 3: Gets actual features from drug_features.csv, as well as adds randomly initialized features equal to extra_dim_embeddings
def get_features(g, extra_dim_embedding=0, seed=42):
    ...
    for ntype in g.ntypes:
        ...
        if ntype == "drug":
            feat = torch.cat((torch.tensor(drug_features.values), feat[:, drug_features.shape[1]:]), dim=1).float().to(device)
        features[ntype] = feat
    return features
...
class HeteroGATLayer(nn.Module):
    def __init__(self, in_feats, out_feats, activation=F.relu):
        ...
        self.conv = dgl.nn.HeteroGraphConv({
            'drug_drug interaction': dgl.nn.GATConv(in_feats, out_feats, allow_zero_in_degree=True, num_heads=3),
            'protein_protein interaction': dgl.nn.GATConv(in_feats, out_feats, allow_zero_in_degree=True, num_heads=3),
            'drug_protein interaction': dgl.nn.GATConv(in_feats, out_feats, allow_zero_in_degree=True, num_heads=3),
            'protein_drug interaction': dgl.nn.GATConv(in_feats, out_feats, allow_zero_in_degree=True, num_heads=3),
            'drug_disease association': dgl.nn.GATConv(in_feats, out_feats, allow_zero_in_degree=True, num_heads=3),
            'disease_drug association': dgl.nn.GATConv(in_feats, out_feats, allow_zero_in_degree=True, num_heads=3),
            'drug_sideeffect association': dgl.nn.GATConv(in_feats, out_feats, allow_zero_in_degree=True, num_heads=3),
            'sideeffect_drug association': dgl.nn.GATConv(in_feats, out_feats, allow_zero_in_degree=True, num_heads=3),
            'protein_disease association': dgl.nn.GATConv(in_feats, out_feats, allow_zero_in_degree=True, num_heads=3),
            'disease_protein association': dgl.nn.GATConv(in_feats, out_feats, allow_zero_in_degree=True, num_heads=3)
        }, aggregate='sum')
    ...
    def forward(self, g, inputs):
        ...
        for key in h.keys():
            h[key] = torch.reshape(h[key], (h[key].shape[0], h[key].shape[1] * h[key].shape[2]))
        for k, v in h.items():
            if self.activation:
...
class HeteroGAT(nn.Module):
    def __init__(self, in_feats, hidden_feats, out_feats, num_heads=3):
        ...
        self.layer1 = HeteroGATLayer(in_feats, hidden_feats)
        self.layer2 = HeteroGATLayer(hidden_feats * num_heads, out_feats)
        self.edge_predictor = nn.Sequential(
            nn.Linear(2 * num_heads * out_feats, 128),
            nn.ReLU(),
            nn.Linear(128, 1),
            nn.Sigmoid()
        )
    ...
    def forward(self, g, features, edge_pairs, etype):
        ...
        src_embeddings = h[etype[0]][edge_pairs[:, 0]].squeeze()
        dst_embeddings = h[etype[2]][edge_pairs[:, 1]].squeeze()
...
class HGT(nn.Module):
    def __init__(self, in_feats, hidden_feats, out_feats, g, num_heads=3):
        super(HGT, self).__init__()
        self.layer1 = HGTLayer(in_feats, hidden_feats * num_heads, g.ntypes, g.canonical_etypes, num_heads=num_heads)
        self.layer2 = HGTLayer(hidden_feats * num_heads, out_feats * num_heads, g.ntypes, g.canonical_etypes, num_heads=num_heads)
        self.edge_predictor = nn.Sequential(
            nn.Linear(2 * num_heads * out_feats, 128),
            nn.ReLU(),
            nn.Linear(128, 1),
    ...
    def forward(self, g, features, edge_pairs, etype):
        ...
        # Edge prediction
        edge_features = torch.cat([src_embeddings, dst_embeddings], dim=1)
        #print(summary(self.edge_predictor, edge_features.shape))
        return self.edge_predictor(edge_features).squeeze()
...
class HeteroGCN(nn.Module):
    ...
    def forward(self, g, features, edge_pairs, etype):
        ...
        print("src_embeddings before", h[etype[0]][edge_pairs[:, 0]].shape)
        print("dst_embeddings before", h[etype[2]][edge_pairs[:, 1]].shape)
        # Gather node embeddings for the specific edge type
        ...
        print("dst_embeddings", dst_embeddings.shape)
...
def train_hetero_gcn(log_file_name, model_file_name, g, features, edge_pairs_dict, labels_dict, canonical_edge_type_dict, epochs=100, patience=10):
    g = g.to(device)
    for ntype in g.ntypes:
    ...
    with open(f'LinkDTI_BBB/model_runs/{log_file_name}.txt', 'w') as f:
        for epoch in range(epochs):
            ...
            if val_loss < best_val_loss:
                ...
                torch.save(best_model_state, "LinkDTI_BBB/models/" + model_file_name + ".pth")
                early_stopping_counter = 0
            else:
        ...
        print(f"Results saved to {log_file_name}.txt.")
...
if __name__ == "__main__":
    ...
    hidden_feats = 256
    out_feats = 128
    model = HGT(in_feats, hidden_feats, out_feats, g)
    ...
    train_hetero_gcn("1_to_1_neg_no_rand_features_HGT_3_head", "1_to_1_neg_no_rand_features_HGT_3_head", g, features, edge_pairs_dict, labels_dict, canonical_edge_type_dict, epochs=100, patience=10)
