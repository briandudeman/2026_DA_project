...
drug_similarity = np.loadtxt('LinkDTI_heterogenous_KG_DA_project/data/Similarity_Matrix_Drugs.txt')
protein_similarity = np.loadtxt('LinkDTI_heterogenous_KG_DA_project/data/Similarity_Matrix_Proteins.txt')
...
def initialize_features(g, dim_embedding=9, seed=42):
...
class HeteroGATLayer(nn.Module):
    def __init__(self, in_feats, out_feats, activation=F.relu):
        ...
        self.conv = dgl.nn.HeteroGraphConv({
            'drug_drug interaction': dgl.nn.GATConv(in_feats, out_feats, allow_zero_in_degree=True, num_heads=1),
            'protein_protein interaction': dgl.nn.GATConv(in_feats, out_feats, allow_zero_in_degree=True, num_heads=1),
            'drug_protein interaction': dgl.nn.GATConv(in_feats, out_feats, allow_zero_in_degree=True, num_heads=1),
            'protein_drug interaction': dgl.nn.GATConv(in_feats, out_feats, allow_zero_in_degree=True, num_heads=1),
            'drug_disease association': dgl.nn.GATConv(in_feats, out_feats, allow_zero_in_degree=True, num_heads=1),
            'disease_drug association': dgl.nn.GATConv(in_feats, out_feats, allow_zero_in_degree=True, num_heads=1),
            'drug_sideeffect association': dgl.nn.GATConv(in_feats, out_feats, allow_zero_in_degree=True, num_heads=1),
            'sideeffect_drug association': dgl.nn.GATConv(in_feats, out_feats, allow_zero_in_degree=True, num_heads=1),
            'protein_disease association': dgl.nn.GATConv(in_feats, out_feats, allow_zero_in_degree=True, num_heads=1),
            'disease_protein association': dgl.nn.GATConv(in_feats, out_feats, allow_zero_in_degree=True, num_heads=1)
        }, aggregate='sum')
...
class HeteroGAT(nn.Module):
    def __init__(self, in_feats, hidden_feats, out_feats):
        ...
        self.layer2 = HeteroGATLayer(hidden_feats, out_feats)
        self.edge_predictor = nn.Sequential(
            nn.Linear(2 * out_feats, 128),
            nn.ReLU(),
            nn.Linear(128, 1),
            nn.Sigmoid()
        )
...
class HGT(nn.Module):
    def __init__(self, in_feats, hidden_feats, out_feats, g):
        super(HGT, self).__init__()
        self.layer1 = HGTLayer(in_feats, hidden_feats, g.ntypes, g.canonical_etypes, num_heads=1)
        self.layer2 = HGTLayer(hidden_feats, out_feats, g.ntypes, g.canonical_etypes, num_heads=1)
        self.edge_predictor = nn.Sequential(
            nn.Linear(2 * out_feats, 128),
            nn.ReLU(),
            nn.Linear(128, 1),
...
def train_hetero_gcn(log_file_name, model_file_name, g, features, edge_pairs_dict, labels_dict, canonical_edge_type_dict, epochs=100, patience=10):
    ...
    with open(f'LinkDTI_heterogenous_KG_DA_project/{log_file_name}.txt', 'w') as f:
        for epoch in range(epochs):
            ...
            if val_loss < best_val_loss:
                ...
                torch.save(best_model_state, "LinkDTI_heterogenous_KG_DA_project/models/" + model_file_name + ".pth")
                early_stopping_counter = 0
            else:
...
train_hetero_gcn("1_to_1_neg_9_rand_features_HGT", "1_to_1_neg_9_rand_features_HGT", g, features, edge_pairs_dict, labels_dict, canonical_edge_type_dict, epochs=100, patience=10)
