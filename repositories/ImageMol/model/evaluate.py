...
def metric(y_true, y_pred, y_prob, empty=-1):
    ...
    precision_recall_curve = metrics.precision_recall_curve(y_true, y_prob)
    ...
    roc_curve = metrics.roc_curve(y_true, y_prob)
    ...
    bacc2 = (metrics.recall_score(y_true, y_pred, pos_label=1) +
            metrics.recall_score(y_true, y_pred, pos_label=0)) / 2

    return {"ROCAUC": roc_auc, "ROC_CURVE": roc_curve, "BACC": balanced_acc, "ACC": acc, "SPEC": specificity_score, "F1": f1_score, "PR_CURVE": precision_recall_curve, "PR_AUC": pr_auc, "PRECISION": precision_score, "RECALL": recall_score, "MSE": mse, "MAE": mae, "RMSE": rmse}
