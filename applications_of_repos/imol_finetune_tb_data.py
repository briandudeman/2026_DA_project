...
from repositories.ImageMol.dataloader.image_dataloader import ImageDataset, load_filenames_and_labels_multitask, get_datasets

sys.path.insert(0, os.path.abspath(os.path.join(root_dir, "repositories", "ImageMol")))
from repositories.ImageMol.model.cnn_model_utils import load_model, train_one_epoch_multitask, evaluate_on_multitask, save_finetune_ckpt
from repositories.ImageMol.model.train_utils import fix_train_random_seed, load_smiles
from repositories.ImageMol.utils.public_utils import cal_torch_model_params, setup_device, is_left_better_right
from repositories.ImageMol.utils.splitter import split_train_val_test_idx, split_train_val_test_idx_stratified, scaffold_split_train_val_test, \
    random_scaffold_split_train_val_test, scaffold_split_balanced_train_val_test

from compare_ImageMol_TitanBBB import imol_process_dataset
...
def parse_args():
    ...
    parser.add_argument('--data_root', type=str, default="./data_process/data/", help='data root')
    ...
    parser.add_argument('--split', default="no_split", type=str,
                        choices=['random', 'stratified', 'scaffold', 'random_scaffold', 'scaffold_balanced', 'no_split'],
...
def main(args):
    ...
    imol_process_dataset(args.data_root, args.dataset, "label", "SMILES", index_name=None)
    ...
    if args.split == "random":
        ...
        train_idx, val_idx, test_idx = scaffold_split_balanced_train_val_test(list(range(0, len(names))), smiles,
                                                                              ...
                                                                              frac_test=0.1, seed=args.seed,
                                                                              balanced=True)
    elif args.split == "no_split":
        smiles = load_smiles(args.txt_file)
        
        train_idx, val_idx, test_idx = scaffold_split_balanced_train_val_test(list(range(0, len(names))), smiles,
                                                                              frac_train=1, frac_valid=0,
                                                                              frac_test=0, seed=args.seed,
                                                                              balanced=True)
